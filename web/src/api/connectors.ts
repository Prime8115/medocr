...
import type { Delivery } from './documents';
...
export async function updateConnector(id: string, body: Partial<ConnectorCreate>): Promise<Connector> {
  const res = await api.patch(`/v1/connectors/${id}`, body);
  return res.data as Connector;
}
...
export async function connectorDeliveries(id: string): Promise<Delivery[]> {
  const res = await api.get(`/v1/connectors/${id}/deliveries`);
